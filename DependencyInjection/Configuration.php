        ...
        $treeBuilder->getRootNode()
            ->children()
                ...
                ->scalarNode('translation_domain')
                    ->info('Translation domain the `datatable.*` keys live in — the ones the shipped partial, the bulk bar and the tenant column read. Defaults to "messages"; a project that splits its catalogues by functional domain sets "datatable" and puts translations/datatable.<locale>.yaml next to its others. The `modal.*` keys are NOT covered: they are a shared modal vocabulary used far beyond tables, and they stay where the application put them.')
                ...
                ->arrayNode('csrf')
                    ...
                    ->children()
                        ...
                        ->scalarNode('bulk')
                            ...
                            ->cannotBeEmpty()
                        ->end()
                        ->scalarNode('preferences')
                            ->info('Token id for the per-user preferences endpoint (PUT/DELETE), sent in the X-CSRF-Token header. A surface of its own rather than a reuse of "single": the writes it guards are personal preferences, not business state transitions.')
                            ->defaultValue('datatable_preferences')
                            ->cannotBeEmpty()
                        ->end()
                    ->end()
                ->end()
                ->arrayNode('status_maps')
                    ->info('Named enum catalogues, DECLARED so the JavaScript translation guard knows their keys are read at runtime. Business vocabulary, hence configuration: the bundle ships none.')
                    ->useAttributeAsKey('name')
                    ->arrayPrototype()
                        ->children()
                            ->scalarNode('key_prefix')
                                ->info('Prefix prepended to each case to form the CATALOGUE key. Defaults to "datatable.<name>.", which is also what the badge renderer is given as its labelPath.')
                                ->defaultNull()
                            ->end()
                            ->arrayNode('keys')
                ...
                ->arrayNode('tenant')
                    ...
                    ->children()
                        ...
                        ->scalarNode('label_domain')
                            ->info('Translation domain for label_key. Defaults to `translation_domain`.')
                            ->defaultNull()
                        ->end()
                    ->end()
                ->end()
