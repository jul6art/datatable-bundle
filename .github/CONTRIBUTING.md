# Contributing to `jul6art/datatable-bundle`

`jul6art/datatable-bundle` is a server-driven table over an API Platform collection:
pagination, sorting, global search, column filters, per-row and bulk actions, confirmation
modals, live refresh over Mercure, and per-user column and view preferences — declared in
PHP, drawn by one Stimulus controller. It is extracted from an application that runs sixty
of them.
...
* **Bug** → open an [issue](https://github.com/jul6art/datatable-bundle/issues/new/choose)
...
```bash
git clone https://github.com/jul6art/datatable-bundle.git
cd datatable-bundle
composer install
```
...
| `test` | PHPUnit (10 test classes today) | by hand |
...
`jul6art/datatable-bundle` is installed by other projects through Composer, so the version
...
2. **Pagination, sorting, searching and filtering are the API's, not a second
   implementation.** The whole point of the bundle is that the table asks an API Platform
   collection; reimplementing any of it here would mean two sets of rules and one of them
   unguarded.

3. **A bulk action decides per row.** Selecting five hundred rows does not authorise five
   hundred writes: the decision is taken for each subject, and a partial refusal is reported
   rather than swallowed.

4. **A column filter only reaches a property the resource exposes**, through the resource's
   own filters. A filter that can be pointed anywhere turns a table into a read primitive
   over the database.

5. **Preferences are per user, and a user only ever reads and writes their own.** Columns,
   order, named views: the bundle interprets them, the application stores them, and the
   identity comes from the session, never from the request.

6. **A live refresh reloads a page of data; it does not carry the data.** Row content
   arrives through the API, which is where the access decision lives — so a Mercure message
   can never show a subscriber what the API would refuse them.

7. **Everything rendered is escaped.** A column label, a formatted cell, an action title and
   a confirmation message all come from the application; the table draws them as text unless
   the declaration explicitly says otherwise.

8. **Interface strings are translation keys, translated in both `en` and `fr`**
   (`translations/`). A hardcoded string in a template is a bug in this repository.

9. **The shipped front-end assets stay build-free for the bundle itself.** There is no
...
10. **`level: max`, and a baseline entry is a last resort.** A `mixed` reaching a Doctrine
...
11. **A new runtime dependency is a discussion, not a commit.** Everything in `require` is
...
12. **Nothing sensitive reaches a log or an exception message** — no password, no token, no
